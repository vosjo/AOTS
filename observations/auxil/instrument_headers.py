...
def derive_eso_info(header):
    ...
    if 'ESO TEL AMBI FWHM END' in header and 'ESO TEL AMBI FWHM START' in header:
        data['seeing'] = np.average(
            [header.get('ESO TEL AMBI FWHM END', -1), header.get('ESO TEL AMBI FWHM START', -1)])
...
def derive_LAMOST_info(header):
    ...
    data['barycor_bool'] = True
    data['observer'] = 'UK'
    data['filetype'] = str(header.get('DATA_V', '').replace(' ', '_')) + '_' + str(
        header.get('ORIGIN', '')) + '_' + str(header.get('OBSID', ''))
