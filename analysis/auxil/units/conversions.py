...
def convert(_from, _to, *args, **kwargs):
    ...
    # -- otherwise a little bit more complicated
        ...
        only_from_c, only_to_c = sorted(list(set(uni_from_) - set(uni_to_))), sorted(
            list(set(uni_to_) - set(uni_from_)))
        only_from_c, only_to_c = [list(components(i))[1:] for i in only_from_c], [list(components(i))[1:] for i in
                                                                                  only_to_c]
...
class AmplMag(NonLinearConverter):
    ...
    def __call__(self, meas, inv=False):
        ...
        if not inv:
            return 10 ** (meas * self.prefix / 2.5) - 1.
        else:
            return (2.5 * log10(1. + meas)) / self.prefix
...
class Unit(object):
    ...
    def sin(self):
        return Unit(sin(self.convert('rad').value), '')

    def cos(self):
        return Unit(cos(self.convert('rad').value), '')

    def tan(self):
        return Unit(tan(self.convert('rad').value), '')

    def arcsin(self):
        return Unit(arcsin(self.value), 'rad')

    def arccos(self):
        return Unit(arccos(self.value), 'rad')

    def arctan(self):
        return Unit(arctan(self.value), 'rad')

    def log10(self):
        return Unit(log10(self.value), '')

    def log(self):
        return Unit(log(self.value), '')

    def exp(self):
        return Unit(exp(self.value), '')
...
_conventions = {'SI': dict(mass='kg', length='m', time='s', temperature='K',
                           electric_current='A', lum_intens='cd', amount='mol', currency='EUR'),
                ...
                'imperial': dict(mass='lb', length='yd', time='s', temperature='K',
                                 electric_current='A', lum_intens='cd', amount='mol', currency='GBP'),
                # Imperial (UK/US) system
                }
