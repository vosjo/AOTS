...
$(document).ready(function() {
   ...
   var edit_lightcurve_window = $("#lightcurveEdit").dialog({autoOpen: false, 
            width: 'auto',
            modal: true});
...
function openLichtCurveEditBox() {
   edit_lightcurve_window = $("#lightcurveEdit").dialog({
      ...
      close: function() { edit_lightcurve_window.dialog( "close" ); }
   });
...
function editLichtCurve() {
   ...
   $.ajax({
      url : "/api/observations/lightcurves/"+lightcurve_id+'/',
      type : "PATCH",
      data : { valid: $("#lightcurve-valid").is(':checked'),
