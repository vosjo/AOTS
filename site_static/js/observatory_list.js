...
$(document).ready(function () {
    ...
    var observatory_window = $("#observatoryEdit").dialog({
        autoOpen: false,
        width: 'auto',
        modal: true
    });
    ...
    observatory_table = $('#observatorytable').DataTable({
        serverSide: true,
        ajax: {
            url: '/api/observations/observatories/?format=datatables&keep=short_name,url',
            data: function (d) {
    ...
    function name_render(data, type, full, meta) {
        if (~full['url'].indexOf("http") || ~full['url'].indexOf("https")) {
            return "<a href='" + full['url'] + "' >" + data + "</a>"
        } else {
            return "<a href='" + 'http://' + full['url'] + "' >" + data + "</a>"
        }
...
function openObservatoryEditBox(tabelrow, data) {
    ...
    $("#id_telescopes").val(data['telescopes']);
    $("#id_latitude").val(data['latitude']);
    $("#id_longitude").val(data['longitude']);
    $("#id_altitude").val(data['altitude']);
    $("#id_space_craft").prop('checked', data['space_craft']);
    $("#id_note").val(data['note']);
    $("#id_url").val(data['url']);
    $("#id_weatherurl").val(data['weatherurl']);
}
