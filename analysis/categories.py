"""
Fixed dataset category registry (replaces per-project Method entities).
"""
...
CATEGORY_META: dict[str, CategoryMeta] = {
    DatasetCategory.RV_SOLUTION: CategoryMeta(
        label='RV solution',
        color='#1e88e5',
        data_type=GENERIC,
        file_type_aliases=frozenset({'RV', 'rv', 'rv_solution'}),
    ),
    DatasetCategory.RV_CURVE: CategoryMeta(
        label='RV curve',
        color='#26a69a',
        data_type=GENERIC,
        file_type_aliases=frozenset({'RC', 'rv_curve', 'rvcurve'}),
    ),
    DatasetCategory.SED_FIT: CategoryMeta(
        label='SED fit',
        color='#8e24aa',
        data_type=SED,
...
def resolve_category(file_type: str | None) -> tuple[str, str]:
    ...
    if not file_type or not str(file_type).strip():
        return DatasetCategory.UNKNOWN, CategorySource.AUTO
...
def category_label(code: str | None) -> str:
    if not code:
        return CATEGORY_META[DatasetCategory.UNKNOWN].label
...
def category_color(code: str | None) -> str:
    if not code:
        return CATEGORY_META[DatasetCategory.UNKNOWN].color
    meta = CATEGORY_META.get(code)
    return meta.color if meta else CATEGORY_META[DatasetCategory.UNKNOWN].color
...
def choices_for_api() -> list[dict[str, str]]:
    return [
        ...
        for code, meta in CATEGORY_META.items()
        if code != DatasetCategory.UNKNOWN
    ] + [
        {
            'value': DatasetCategory.UNKNOWN,
            'label': CATEGORY_META[DatasetCategory.UNKNOWN].label,
            'color': CATEGORY_META[DatasetCategory.UNKNOWN].color,
            'data_type': CATEGORY_META[DatasetCategory.UNKNOWN].data_type,
        },
    ]
