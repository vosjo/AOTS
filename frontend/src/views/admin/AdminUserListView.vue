...
<template>
  ...
  >
    <template #actions>
      <RouterLink to="/admin/users/new" class="aots-btn-primary inline-flex items-center gap-1 text-sm">
        <Plus class="h-4 w-4" />
        Add user
      </RouterLink>
    </template>
    ...
    <template #cell-actions="{ row }">
      <RouterLink :to="`/admin/users/${rowPk(row)}`" class="text-sm text-sky-400 hover:text-sky-300">
        Edit
      </RouterLink>
    </template>
  </DataTablePage>
